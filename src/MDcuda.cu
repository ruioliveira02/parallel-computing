...
int main()
{
    ...
    v_cpu = (double*)malloc(3 * N * sizeof(double));
    a_cpu = (double*)malloc(3 * N * sizeof(double));
    r_cpu = (double*)malloc(3 * N * sizeof(double));
        ...
        for (j = 0; j < n; j++) {
            for (k = 0; k < n; k++) {
                if (p < N) {
                    ...
                    r_cpu[3 * p] = (i + 0.5) * pos;
                    r_cpu[3 * p + 1] = (j + 0.5) * pos;
                    r_cpu[3 * p + 2] = (k + 0.5) * pos;
                }
                p++;
            }
        }
...
double MeanSquaredVelocity() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        vx2 = vx2 + v_cpu[3 * i] * v_cpu[3 * i];
        vy2 = vy2 + v_cpu[3 * i + 1] * v_cpu[3 * i + 1];
        vz2 = vz2 + v_cpu[3 * i + 2] * v_cpu[3 * i + 2];
...
__global__ void updatePositionsKernel(double *a, double *v, double *r, double dt) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void updateVelocityKernel(double *a, double *v, double *r, double dt, double l) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
...
double VelocityVerlet(double dt, int iter, FILE* fp, double* potential) {
    ...
    *potential = computeAccelerations();
    ...
    for (int i = 0; i < 3 * N; i++) {
        if (r_cpu[i] < 0.) {
            psum += 2 * m * fabs(v_cpu[i]) / dt;  // contribution to pressure from "left" walls
        }
        if (r_cpu[i] >= L) {
            psum += 2 * m * fabs(v_cpu[i]) / dt;  // contribution to pressure from "right" walls
        }
    }
...
void initializeVelocities() {
    ...
    for (i = 0; i < N; i++) {
        for (k = 0; k < 3; k++) {
            ...
            vSqdSum += v_cpu[3 * i + k] * v_cpu[3 * i + k];
