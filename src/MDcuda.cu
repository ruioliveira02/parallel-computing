...
{    
	...
	cudaMalloc ((void**) &dr, 3 * N * sizeof(double));
	cudaMalloc ((void**) &da, 3 * N * sizeof(double));
...
__global__ void computeAccelerationsKernel(double *r, double *a, double *Pot, int N) {
...
void copyInput() {
	...
	cudaMemcpy (dr, r_aux, 3 * N * sizeof(double), cudaMemcpyHostToDevice);
...
double copyOutput() {
    ...
    double a_aux[3*N], Pot_aux[N];
	cudaMemcpy (a_aux, da, 3 * N * sizeof(double), cudaMemcpyDeviceToHost);
...
double computeAccelerations() {
	...
	computeAccelerationsKernel <<< threads_per_block, num_blocks >>> (dr, da, dPot, N);
